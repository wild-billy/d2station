...
/obj/item/device/radio/proc/autosay(var/message,var/from,var/channel)
	...
	for(var/obj/item/device/radio/portajammer/T in range(2))
		if ((T.frequency == connection.frequency) && (T.on == 1))
			return
...
//		world << "finds secure radio connection]"
	else
		connection = radio_connection
//		world << "finds radio connection]"
		channel = null
	if (!istype(connection))
		return

//	world << "connection frequency is [connection.frequency]"
...
/obj/item/device/radio/talk_into(mob/M as mob, message, channel)
	for(var/obj/machinery/radiotransmitter/TR in machines)
		if ((src.requiretransmitter == 0) || TR.transmitteron == 1)
			...
			//If a portajammer is in place, deny signals
			for(var/obj/item/device/radio/portajammer/T in range(2))
				if ((T.frequency == connection.frequency) && (T.on == 1))
					return
			//if a portajammer is in place, deny signals
			...
			//		var/J = eqjobname
					...
					//Would not be necessary but the mob could be wearing a mask that is not a voice changer.
						N = M.wear_mask:voice
			//			J = "Unknown"
					var/rendered = "[part_a]\[[eqjobname]\] [N][part_b][quotedmsg][part_c]"
					for (var/mob/R in heard_masked)
						if(istype(R, /mob/living/silicon/ai))
							R.show_message("[part_a]<a href='byond://?src=\ref[src];track2=\ref[R];track=\ref[M]'>[N] </a>[part_b][quotedmsg][part_c]", 2)
						else
							R.show_message(rendered, 2)

				if (length(heard_normal))
					var/rendered = "[part_a]\[[eqjobname]\] [M.real_name][part_b][quotedmsg][part_c]"

					for (var/mob/R in heard_normal)
						if(istype(R, /mob/living/silicon/ai))
							R.show_message("[part_a]\[[eqjobname]\] <a href='byond://?src=\ref[src];track2=\ref[R];track=\ref[M]'>[M.real_name]  </a>[part_b][quotedmsg][part_c]", 2)
						else
							R.show_message(rendered, 2)

				if (length(heard_voice))
					var/rendered = "[part_a]\[[eqjobname]\] [M.voice_name][part_b][M.voice_message][part_c]"

					for (var/mob/R in heard_voice)
						if(istype(R, /mob/living/silicon/ai))
							R.show_message("[part_a]\[[eqjobname]\] <a href='byond://?src=\ref[src];track2=\ref[R];track=\ref[M]'>[M.voice_name]  </a>[part_b][M.voice_message][part_c]", 2)
						else
							R.show_message(rendered, 2)

				if (length(heard_garbled))
					quotedmsg = M.say_quote(stars(message))
					var/rendered = "[part_a]\[[eqjobname]\] [M.voice_name][part_b][quotedmsg][part_c]"

					for (var/mob/R in heard_voice)
						if(istype(R, /mob/living/silicon/ai))
							R.show_message("[part_a]\[[eqjobname]\] <a href='byond://?src=\ref[src];track2=\ref[R];track=\ref[M]'>[M.voice_name]</a>[part_b][quotedmsg][part_c]", 2)
						else
							R.show_message(rendered, 2)
