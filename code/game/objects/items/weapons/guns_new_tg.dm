...
/obj/item/projectile
	...
	phaser
		...
		proc/effect_tiles(var/effect_type)
			if(effect_type == "stun")
				for (var/turf/phaseturf in range(radius,src))
					for(var/mob/living/carbon/M in phaseturf)
						M.stunned += power / 5
						M.weakened += power / 5
					new /obj/effects/sparks(phaseturf.loc)
				del(src)
				return
		...
		spawn(0)
			if(A)
				...
				if(istype(src, /obj/item/projectile/freeze))
					...
					Q.effect_tiles(Q.loaded_effect)
				else
	...
	process()
		spawn while(src)
			...
			if(!bumped)
				if(loc == original)
					if(istype(src, /obj/item/projectile/phaser))
						var/obj/item/projectile/phaser/Q = src
						Q.effect_tiles()
...
/obj/item/ammo_casing
	...
	cT38
		name = "bullet casing (.38 taser)"
		desc = "A .38 taser casing."
		icon_state = "t-casing"
		caliber = "38"

		New()
			BB = new /obj/item/projectile/electrode(src)
			pixel_x = rand(-10.0, 10)
			pixel_y = rand(-10.0, 10)
			dir = pick(cardinal)
	...
	magnetpellet
		...
		New()
			BB = new /obj/item/projectile/magnet(src)
			del(src)
...
/obj/item/ammo_magazine
	...
	cT38
		name = "speed loader (.38 taser bullet)"
		icon_state = "tase38"
		New()
			for(var/i = 1, i <= 6, i++)
...
/obj/item/weapon/gun
		...
		phaser
			...
			proc/Life()
				while(src)
					...
					switch(temperature)
						if(601 to 800) charge_cost = 150
						if(401 to 600) charge_cost = 100
						if(201 to 400) charge_cost = 75
						if(101 to 200) charge_cost = 50
						if(51 to 100) charge_cost = 25
						if(0 to 50) charge_cost = 1
	...
	afterattack(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, flag)
		...
		///start of checks for sec implant.

		if(!user.authenticsec && istype(src, /obj/item/weapon/gun/energy/phaser))
			user << "\red The [src.name] makes an annoyed growning sound, you must lack an implant."
			return
...
//			in_chamber:loaded_effect = src:loaded_effect
