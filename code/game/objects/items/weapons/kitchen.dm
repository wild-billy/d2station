...
// RAZORBLADE
/obj/item/weapon/kitchen/utensil/razorblade/attack(target as mob, mob/living/user as mob)
	user << "\red You cut yourself with the [src]."

	if(!(user.zone_sel.selecting == ("eyes" || "head"))) //////////////hitting anything else other than the eyes
		src.add_blood(user)
		var/turf/location = user.loc
		if (istype(location, /turf/simulated))
			location.add_blood(user)     ///Plik plik, the sound of blood
		user.take_organ_damage(1) //changed from 10 because people who cut just want attention, not death. Although you can still kill yourself with it. - Nernums
	return							//although we should really fix this all to work with the bleeding out system.
...
/obj/item/weapon/tray/attack(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
	if((user.mutations & CLOWN) && prob(50))              //What if he's a clown?
		...
		if(prob(50))
			playsound(M, 'trayhit2.wav', 50, 1)
			return
		else
			playsound(M, 'trayhit1.wav', 50, 1) //sound playin'
	...
	if(!(user.zone_sel.selecting == ("eyes" || "head"))) //////////////hitting anything else other than the eyes
		...
		if(prob(50))
			...
			playsound(M, 'trayhit1.wav', 50, 1)  //we applied the damage, we played the sound, we showed the appropriate messages. Time to return and stop the proc
			for(var/mob/O in viewers(M, null))
				O.show_message(text("\red <B>[] slams [] with the tray!</B>", user, M), 1)
			return
	...
	if(istype(M, /mob/living/carbon/human) && ((H.head && H.head.flags & HEADCOVERSEYES) || (H.wear_mask && H.wear_mask.flags & MASKCOVERSEYES) || (H.glasses && H.glasses.flags & GLASSESCOVERSEYES)))
		...
		if(prob(50))
			...
			playsound(M, 'trayhit1.wav', 50, 1)  //sound playin' again
			for(var/mob/O in viewers(M, null))
				O.show_message(text("\red <B>[] slams [] in the face with the tray!</B>", user, M), 1)
		if(prob(30))
			M.stunned = rand(2,4)
