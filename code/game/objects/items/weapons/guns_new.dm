...
/obj/item/projectile
	...
	process()
		spawn while(src)
			...
			if(istype(src, /obj/item/projectile/beam/phaser))
				world << "Someone fired a phaser gun"
				if(src:target_tile)
					world << "The phaser bullet has a valid target tile"
					var/turf/phasetile = get_turf(src:target_tile)
					var/list/phasturfs = list()
					if(src.loc == phasetile)
						world << "The phaser bullet has reached its target tile"
						phasturfs += phasetile.loc
						world << "Phaser bullet target tile now assigned to phaser-affected tiles"
						if(src:radius)
							world << "The phaser bullet will affect more than one tile"
							for(var/turf/O in orange(src:radius, phasetile))
								phasturfs += O
								world << "[O], [O.x],[O.y] added to phaser-affected tiles"
							if(phasturfs)
								for(var/mob/living/carbon/M in phasturfs)
									M.stunned += src:power / 5
									M.weakened += src:power / 5
									world << "[M] has been affected by the phaser bullet"
								for(var/turf/S in phasturfs)
									new /obj/effects/sparks(S.loc)
									world << "PhasTurf sparked at [S.x], [S.y]"
							del(src)
							return
						else
							world << "The phaser bullet will only affect one tile"
							if(phasturfs)
								for(var/mob/living/carbon/M in phasturfs)
									M.stunned += src:power / 5
									M.weakened += src:power / 5
...
/obj/item/ammo_magazine
	...
	fiveseven
		...
		New()
			src.pixel_x = rand(-10.0, 10)
			for(var/i = 1, i <= 9, i++)
				stored_ammo += new /obj/item/ammo_casing/fiveseven(src)
			update_icon()
...
/obj/item/weapon/gun
			...
			verb
				rename_gun()
					...
					if(ishuman(U)&&U.mind&&U.mind.assigned_role=="Detective")
						var/input = strip_html(input("What do you want to name the gun?",,""))
	...
	energy
		...
		phaser
			...
			load_into_chamber()
				...
				if(power_supply.charge <= charge_cost)
					return 0
				in_chamber = new /obj/item/projectile/beam/phaser(src)
				power_supply.use(charge_cost)
				return 1
			...
			proc/AdjustPowerUse()
				while(src)
					...
					if(power)
						src.charge_cost = ((src.power * 4) * (src.radius + 1))

			attack_self(mob/living/user as mob)
				user.machine = src
				var/dat = {"<B>Phaser Gun Configuration: </B><BR>
				Radius: <A href='?src=\ref[src];radius_adj=-1'>-</A> [src.radius+1] <A href='?src=\ref[src];radius_adj=1'>+</A><BR>
			...
			Topic(href, href_list)
				...
				if (href_list["radius_adj"])
					...
					if(rdiff > 0)
						radius = min(radius+rdiff)
					else
						radius = max(radius-rdiff)
					if(radius < 0)
						radius = 0
					if(radius > 2)
						radius = 2
				if (href_list["power_adj"])
					...
					if(pdiff > 0)
						power = min(power+pdiff)
					else
						power = max(power-pdiff)
					if(power < 5)
						power = 5
					if(power > 40)
						power = 40
	...
	afterattack(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, flag)
		...
		if(istype(src, /obj/item/weapon/gun/energy/phaser))
			in_chamber:target_tile = get_turf(target)
			in_chamber:radius = src:radius
			in_chamber:power = src:power
