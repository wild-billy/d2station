...
/obj/item/proc/attack(mob/living/M as mob, mob/living/user as mob, def_zone)
	...
	if(!M.client)
		if(M.ai_active == 1 && ((istype(M, /mob/living/carbon/human/retard/violent/zombors)&& M.brainloss >= 20) || istype(M, /mob/living/carbon/monkey)))
			spawn(1)
			M:ai_state = 2
			M:ai_target = user
			if(istype(M, /mob/living/carbon/human/retard/violent/zombors) && M.brainloss >= 20)
				M.say(pick("Mmarrgh!", "Rarrgh!", "Graaaagh!", "Wuugh!", "Uurrrgg!"))
		/*else if(M.ai_active == 1 && ((istype(M, /mob/living/carbon/human/zombie) && M.brainloss >= 20) || istype(M, /mob/living/carbon/monkey)))
			spawn(1)
			M:ai_state = 2
			M:ai_target = user
			if(istype(M, /mob/living/carbon/human/zombie))
				M.say(pick("Mmarrgh!", "Rarrgh!", "Graaaagh!", "Wuugh!", "Uurrrgg!"))*/
		else if(M.ai_active == 1 && ((istype(M, /mob/living/carbon/human) && M.brainloss >= 20) || istype(M, /mob/living/carbon/monkey)))
			spawn(1)
			M:ai_state = 2
			M:ai_target = user
			if(istype(M, /mob/living/carbon/human))
				M.say(pick("[user] attacked me!", "help me [user] is attacking me!!", "Help at [M.loc.loc] [user] is killing me!", "[user] is attacking me in [M.loc.loc]!", "need help in [M.loc.loc]!!"))
		...
		if (istype(affecting, /datum/organ/external))
			...
			if (def_zone == "head")
				if (b_dam && prob(armor["armor"] - src.force))
					...
					return
				if ((b_dam && prob(src.force + affecting.brute_dam + affecting.burn_dam) /*&& !H in zombies*/))
...
/obj/item/proc/meltfrozen(mob/living/carbon/M as mob, mob/living/carbon/user as mob)

	if(M.frozen)
		M.frozen--
		M << "\blue You can feel your extremities again."
		for(var/mob/O in (viewers(M) - user - M))
			O.show_message("\blue [M] has had some of \his icy covering melted by [user] with the [src]!", 1)
		user << "\blue You melt the ice covering [M]!"
