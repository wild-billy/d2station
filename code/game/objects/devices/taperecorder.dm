...
/obj/item/device/taperecorder/verb/record()
	...
	if(src.timerecorded < 360 && src.playing == 0)
		src.recording = 1
		for(src.timerecorded, src.timerecorded<360)
			if(src.recording == 0)
				break
			src.timerecorded++
			sleep(10)
...
/obj/item/device/taperecorder/verb/playback_memory()
	...
	for(var/i=1,src.timerecorded<360,sleep(5 * (src.timerecorded/src.storedinfo.len)))
		if (src.playing == 0)
			break
