/obj/item/mecha_parts/mecha_equipment/tool/hydraulic_clamp
	...
	can_attach(obj/mecha/M as obj)
		if(..())
			if(istype(M, /obj/mecha/working/ripley) && (M.leftarm == 0) || (M.rightarm == 0))
				return 1
		return 0
...
/obj/item/mecha_parts/mecha_equipment/tool/hydraulic_lifter
	name = "Robustmobile lifter"
	icon_state = "mecha_clamp"
	equip_cooldown = 15
	energy_drain = 0
	var/dam_force = 20
	var/obj/mecha/working/robustmobile/cargo_holder

	can_attach(obj/mecha/M as obj)
		if(..())
			if(istype(M, /obj/mecha/working/robustmobile))
				return 1
		return 0
	...
	action(atom/target)
		...
		if(do_after_cooldown(target))
			if(T == chassis.loc && src == chassis.selected)
				...
				else if(istype(target, /turf/simulated/wall/asteroid))
					for(var/turf/simulated/wall/asteroid/M in range(chassis,1))
...
/obj/item/mecha_parts/mecha_equipment/tool/extinguisher
	...
	action(atom/target) //copypasted from extinguisher. TODO: Rewrite from scratch.
		...
		set_ready_state(0)
		if(do_after_cooldown(target))
			if((istype(target, /obj/reagent_dispensers/watertank) || (istype(target, /obj/reagent_dispensers/foamtank))) && get_dist(chassis,target) <= 1)
				var/obj/o = target
