/proc/playsound(var/atom/source, soundin, vol as num, vary, extrarange as num)
	...
	switch(soundin)
		...
		if ("buttstep") soundin = pick('fart.ogg','poo2.ogg')
		if ("erikafootsteps") soundin = pick('tile1.wav','tile2.wav','tile3.wav','tile4.wav')
		if ("concretefootsteps") soundin = pick('concrete1.wav','concrete2.wav','concrete3.wav','concrete4.wav')
		if ("grassfootsteps") soundin = pick('grass1.wav','grass2.wav','grass3.wav','grass4.wav')
		if ("dirtfootsteps") soundin = pick('dirt1.wav','dirt2.wav','dirt3.wav','dirt4.wav')
		if ("waterfootsteps") soundin = pick('slosh1.wav','slosh2.wav','slosh3.wav','slosh4.wav')
		if ("sandfootsteps") soundin = pick('sand1.wav','sand2.wav','sand3.wav','sand4.wav')
	...
	for (var/mob/M in range(10+extrarange, source))
		if (M.client)
			if(isturf(source))
				var/dx = source.x - M.x
...
/mob/proc/playsound_local(var/atom/source, soundin, vol as num, vary, extrarange as num)
	...
	switch(soundin)
		...
		if ("punch") soundin = pick('punch1.ogg','punch2.ogg','punch3.ogg','punch4.ogg')
		if ("clownstep") soundin = pick('clownstep1.ogg','clownstep2.ogg')
		if ("buttstep") soundin = pick('fart.ogg','squishy.ogg')
		if ("erikafootsteps") soundin = pick('footsteps.ogg','footsteps2.ogg')
		if ("swing_hit") soundin = pick('genhit1.ogg', 'genhit2.ogg', 'genhit3.ogg')
		if ("hiss") soundin = pick('hiss1.ogg','hiss2.ogg','hiss3.ogg','hiss4.ogg')
