/obj/machinery/atmospherics/unary/vent_scrubber
	...
	initialize()
		...
		alarm_area.scrubbers += src

		if (name == initial(name))
			name = "[alarm_area.name] air scrubber #[alarm_area.scrubbers.len]"

	update_icon()
		if(node && is_operating())
			if(scrubbing)
	...
	proc
		apply_atmospherics_mode()
			...
			on = (mode != ATMOS_MODE_OFF)
			scrubbing = (mode == ATMOS_MODE_SCRUBBING)
			panic = (mode == ATMOS_MODE_PANIC || mode == ATMOS_MODE_REPLACEMENT)
	...
	process()
		...
		var/volume_rate = (panic ? 2000 : 520)
