/obj/machinery/space_cooler
	...
	icon_state = "scooler0"
	name = "space cooler"
	desc = "Made by Space Amish using traditional space techniques, this cooler is guaranteed not to set the station on ice."
	var/obj/item/weapon/cell/cell
	var/on = 0
	var/open = 0
	var/set_temperature = 250-T0C		// in celcius, add T0C for kelvin
	...
	attack_hand(mob/user as mob)
		...
		if(open)
			...
			user.machine = src
			user << browse("<HEAD><TITLE>Space Heater Control Panel</TITLE></HEAD><TT>[dat]</TT>", "window=spacecooler")
		...
		if ((in_range(src, usr) && istype(src.loc, /turf)) || (istype(usr, /mob/living/silicon)))
			...
			switch(href_list["op"])
				...
				if("temp")
					...
					// limit to -60 to 15 degC
					set_temperature = dd_range(200-T0C, 271-T0C, set_temperature + value)
					...
					if(env.temperature > (set_temperature+T0C))
						...
						if(removed)
							...
							if(heat_capacity == 0 || heat_capacity == null) // Added check to avoid divide by zero (oshi-) runtime errors -- TLE
								heat_capacity = 1
							removed.temperature = min((removed.temperature*heat_capacity - cooling_power)/heat_capacity, 1000) // Added min() check to try and avoid wacky superheating issues in low gas scenarios -- TLE
