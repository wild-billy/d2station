...
/obj/machinery/microwave/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(src.broken > 0)
		...
		if(istype(O, /obj/item/weapon/spraybottle/cleaner)) // If they're trying to clean it then let them
			user.visible_message( \
...
/obj/machinery/microwave/proc/interact(mob/user as mob) // The microwave Menu
	...
	if(src.broken > 0)
		...
		for (var/obj/O in contents)
			...
			if (istype(O,/obj/item/weapon/reagent_containers/food/snacks/dongpocket))
				display_name = "Turnovers"
				items_measures[display_name] = "turnover"
				items_measures_p[display_name] = "turnovers"
