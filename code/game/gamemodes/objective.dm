	...
	objective
		...
		steal
			...
			var/global/possible_items_special[] = list(
				...
				"diamond drill" = /obj/item/weapon/tgpickaxe/diamonddrill,
			...
			proc/select_target()
				...
				if (new_target == "custom")
					...
					del(tmp_obj)
					custom_name = strip_html(input("Enter target name:", "Objective target", custom_name) as text|null)
			...
			proc/gen_amount_goal()  //this doesn't work -- should work now, changed it a bit -- Urist
				...
				if (ticker)
					...
					if (ticker.current_state == GAME_STATE_SETTING_UP)
						...
						for(var/mob/living/carbon/human/P in mobz)
