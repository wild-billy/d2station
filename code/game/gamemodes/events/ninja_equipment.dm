	...
	switch(spideros)
		...
		if(3)
			...
			for(var/datum/disease/D in U.viruses)
				dat += "Warning: Ailment Detected. Name: [D.name].Type: [D.spread]. Stage: [D.stage]/[D.max_stages]. Possible Cure: [D.cure].<br>"
			dat += "<ul>"
...
/obj/item/clothing/suit/space/space_ninja/Topic(href, href_list)
	...
	switch(href_list["choice"])
		...
		if("Message")
			...
			var/t = strip_html(input(U, "Please enter untraceable message.") as text)
