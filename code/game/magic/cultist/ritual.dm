	...
	Topic(href,href_list[])
		if (src.loc == usr)
			...
			switch(href_list["action"])
				...
				if("change")
					words[words[number]] = strip_html(input("Enter the translation for [words[number]]", "Word notes") in engwords)
					for (var/w in words)
...
//						call(/obj/item/weapon/tome/proc/edit_notes)()
			...
			for (var/w in words)
				english+=words[w]
			if(usr)
				w1 = strip_html(input("Write your first rune:", "Rune Scribing") in english)
				for (var/w in words)
					if (words[w] == w1)
						w1 = w
			if(usr)
				w2 = strip_html(input("Write your second rune:", "Rune Scribing") in english)
				for (var/w in words)
					if (words[w] == w2)
						w2 = w
			if(usr)
				w3 = strip_html(input("Write your third rune:", "Rune Scribing") in english)
				for (var/w in words)
					if (words[w] == w3)
						w3 = w
			for (var/mob/V in viewers(src))
...
/obj/item/weapon/tome/imbued //admin tome, spawns working runes without waiting
	...
	attack_self(mob/user as mob)
		...
		if(user)
			...
			r = strip_html(input("Choose a rune to scribe", "Rune Scribing") in runes) //not cancellable.
			...
			switch(r)
				if("teleport")
					...
					var/beacon
					if(usr)
						beacon = strip_html(input("Select the last rune", "Rune Scribing") in words)
					R.word1=wordtravel
					R.word2=wordself
					R.word3=beacon
					R.loc = user.loc
...
/obj/item/weapon/paperscrap
	...
	proc/view_scrap(var/viewer)
		viewer << browse(data, "window=paperscrap")
