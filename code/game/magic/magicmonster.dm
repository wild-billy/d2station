			...
			if(can_see(src,target,viewrange))
				if(distance <= 1)
					for(var/mob/O in viewers(world.view,src))
						O.show_message("\red <B>[src.target] has been attacked by [src]!</B>", 1, "\red You hear the sounds of robust", 2)
...
/obj/creature/rapemonger
	name = "rapemonger"
	desc = "it hungers for dicks"
	icon = 'otherthing.dmi'
	icon_state = "otherthing"
	cycle_pause = 1

/* TOO LAGGY
/obj/creature/rapemonger/process()
	set background = 1
	var/quick_move = 0
	...
	if (!target)
		...
		if(can_see(src,target,viewrange))
			if(distance <= 1)
				for(var/mob/O in viewers(world.view,src))
					O.show_message("\red <B>[src.target] has been raped by [src]!</B>", 1, "\red You hear the sounds of robust", 2)
					target:removePart(pick("hand_right", "hand_left", "arm_right", "arm_left", "leg_right", "leg_left", "foot_right", "foot_left"))
					target:bruteloss += rand(1,10)
					sleep(5)
