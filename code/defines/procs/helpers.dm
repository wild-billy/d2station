...
/proc/step_towards(var/atom/movable/Ref, var/atom/movable/Trg)
	if (!Ref || !Trg)
		return 0
	if(Ref.z == Trg.z)
		var/S = Ref.loc
	...
	if (istype(Ref, /obj/machinery/bot/secbot))
		...
		while (S.target && get_dist(S,S.target) > min)
			...
			if(istype( next, /turf/simulated))
				var/moved = step_towards(S, next)	// attempt to move
...
/proc/sanitize(var/t,var/list/repl_chars = null)
	return html_decode(sanitize_simple(t,repl_chars))
...
/proc/clname(var/mob/M as mob) //--All praise goes to NEO|Phyte, all blame goes to DH, and it was Cindi-Kate's idea
	...
	if (newname)
		if (newname == "Unknown")
			M << "That name is reserved."
			return clname(M)
		for (var/mob/living/carbon/A in mobz)
...
/proc/freeborg()
	...
	for (var/mob/living/silicon/robot/A in mobz)
...
/proc/sortmobs()
	...
	for(var/mob/living/silicon/ai/M in mobz)
		mob_list.Add(M)
	for(var/mob/living/silicon/pai/M in mobz)
		mob_list.Add(M)
	for(var/mob/living/silicon/robot/M in mobz)
		mob_list.Add(M)
	for(var/mob/living/carbon/human/M in mobz)
		mob_list.Add(M)
	for(var/mob/living/carbon/brain/M in mobz)
		mob_list.Add(M)
	for(var/mob/living/carbon/alien/M in mobz)
		mob_list.Add(M)
	for(var/mob/dead/observer/M in mobz)
		mob_list.Add(M)
	for(var/mob/new_player/M in mobz)
		mob_list.Add(M)
	for(var/mob/living/carbon/monkey/M in mobz)
		mob_list.Add(M)
	for(var/mob/living/silicon/hivebot/M in mobz)
		mob_list.Add(M)
	for(var/mob/living/silicon/hive_mainframe/M in mobz)
		mob_list.Add(M)
	return mob_list
