#define NITROGEN_RETARDATION_FACTOR 0	//Higher == N2 slows reaction more
#define THERMAL_RELEASE_MODIFIER 5000000		//Higher == less heat released during reaction
#define PLASMA_RELEASE_MODIFIER 100	//Higher == less plasma released by reaction
#define OXYGEN_RELEASE_MODIFIER 100		//Higher == less oxygen released at high temperature/power
#define REACTION_POWER_MODIFIER 20	//Higher == more overall power
...
/obj/machinery/engine/klaxon/process()
	if(!sup)
		for(var/obj/machinery/engine/supermatter/T in machines)
			sup = T
			break
	if(sup.det >= 1)
		return
...
/proc/radioalert(var/message,var/from,var/channel)
	var/obj/item/device/radio/headset/heads/captain/a = new /obj/item/device/radio/headset/heads/captain(null)
	a.autosay(message,from,channel)
...
/obj/machinery/engine/supermatter/process()
	...
	previousdet = det
	det += (removed.temperature - 2000) / 150
	det = max(det, 0)
...
//	if(det > 0 && removed.temperature > 1000) // while the core is still damaged and it's still worth noting its status
	...
	//		if(explosiondet - det <= 200)
	//			radioalert("ATTENTION! CORE EXPLOSION IMMINENT","Core control computer")
	//			world << 'warning-buzzer.ogg'
//			else if(det >= previousdet)   // The damage is still going up
	//			radioalert("WARNING: CORE EXCEEDING RECOMMENDED SAFETY LEVELS.","Core control computer","Engineering")
	//		else						  // Phew, we're safe
	//			radioalert("Core returning to safe operating levels.","Core control computer","Engineering")

	if(det > explosiondet)
//		roundinfo.core = 1
		//proc/explosion(turf/epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, force = 0)
		explosion(src.loc,0,0,0,2,6)
		det = 0
...
//		if(prob(10))
//			l << "\red A wave of painful energy strikes you!"
//
//		l.bruteloss += 3
//		l.fireloss += 3
//		l.toxloss += 3
