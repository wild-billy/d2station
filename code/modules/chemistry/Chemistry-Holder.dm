		...
		proc
			...
			copy_to(var/obj/target, var/amount=1, var/multiplier=1, var/preserve_data=1)
				...
				for (var/datum/reagent/current_reagent in src.reagent_list)
					...
					if(preserve_data)
						trans_data = current_reagent.data
					R.add_reagent(current_reagent.id, (current_reagent_transfer * multiplier), current_reagent.current_temp, trans_data)
			...
			handle_reactions()
				...
				do
					...
					for(var/A in typesof(/datum/chemical_reaction) - /datum/chemical_reaction)
						...
						var/total_required_stabilizers = C.required_stabilizers.len
						...
						for(var/B in C.required_stabilizers)
							if(has_reagent(B, C.required_stabilizers[B]))
					...
					//	else
					//		if(istype(my_atom, /obj/item/weapon/reagent_containers/glass/metroidcore))
						//		var/obj/item/weapon/reagent_containers/glass/metroidcore/ = my_atom
						...
						if(total_matching_reagents == total_required_reagents && total_matching_catalysts == total_required_catalysts && matching_container && matching_other)
							...
							if(!total_required_stabilizers == total_matching_stabilizers && (prob(C.volatility * 15)))
								sleep(10)
								var/location = get_turf(my_atom)
								var/datum/effects/system/reagents_explosion/e = new()
								e.set_up(rand(C.volatility,1), location, 0, 0)
								e.start()
								del created_volume
			...
			add_reagent(var/reagent, var/amount, var/data=null)
				...
				for(var/A in reagent_list)
					...
					if (R.id == reagent)
						...
						// mix dem viruses
					//	if(R.name == "vaccine") return

					//	else if((R.data && R.data["viruses"]) || (data && data["viruses"]))
					//		var/list/this = R.data["viruses"]
					//		var/list/that = data["viruses"]
					///		if(this == that)
						//		world << "they der same guiiizzzeee"
					//			return

					//		else
						//		world << "combine Arrrvester"
					//			this -= that
