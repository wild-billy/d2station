...
//		if((flags & SUITSPACE) && !plc.PLASMAGUARD_ONLY) return 1
//		if(plc.ALL_ITEM_CONTAMINATION) return 1
...
/*
	if(istype(back,/obj/item/weapon/storage/backpack) || plc.ALL_ITEM_CONTAMINATION)
		back.contaminated = 1
		...
		if(!pl_head_protected() || !pl_suit_protected())
			...
			if (coughedtime != 1)
				...
				spawn (20)
					coughedtime = 0
			updatehealth()
	if(plc.EYE_BURNS && !pl_head_protected())
		if(!wear_mask)
			...
			if(!(wear_mask.flags & MASKCOVERSEYES))
				...
				if (prob(max(0,eye_stat - 20) + 1) &&!eye_blind)
					...
					eye_blind += 20
					eye_stat = 0
	if(plc.GENETIC_CORRUPTION)
		if(rand(1,1000) < plc.GENETIC_CORRUPTION)
			randmutb(src)
