...
var
	FLOW_PERCENT = 80 //Percent of gas to send between connected turfs.
	VACUUM_SPEED = 45.2 //Divisor of zone gases exposed directly to space (i.e. space tiles in members)
		...
		while(1)
			...
			if(space_connections.len)				 //Throw gas into space if it has space connections.
				...
				for(var/turf/simulated/M in members)
					...
					GM.temperature = round(GM.temperature / VACUUM_SPEED, 0.01)
				oxygen = round(oxygen ,0.01)
				nitrogen = round(nitrogen ,0.01)
				co2 = round(co2 ,0.01)
