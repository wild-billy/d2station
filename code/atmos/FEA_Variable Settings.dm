var/global/vs_control/vs_control
vs_control
	var
		list/settings = list()
	...
	proc/ChangeSetting(mob/user,ch)
		...
		switch(how)
			...
			if("Toggle")
				newvar = !newvar
			if("Text")
				newvar = strip_html(input(user,"Enter a string:","Settings",newvar) as text)
			if("Long Text")
				newvar = strip_html(input(user,"Enter text:","Settings",newvar) as message)
		vw = newvar
		if(ch in plc.settings)
			plc.vars[ch] = vw
		else
