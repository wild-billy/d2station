...
turf
	...
	simulated
		hotspot_expose(exposed_temperature, exposed_volume, soh)
			...
			if(active_hotspot)
				if(soh)
					if(air_contents.toxins > 1.5 && air_contents.oxygen > 0.05)
			...
			if(igniting)
				if(air_contents.oxygen < 0.05 || air_contents.toxins < 1.5)
					return 0
...
obj
	hotspot
		...
		proc/perform_exposure()
			...
			if(bypassing)
				if(!just_spawned)
					volume = round(location.air.fuel_burnt*FIRE_GROWTH_RATE, 0.01)
				...
				volume = round(affected.fuel_burnt*FIRE_GROWTH_RATE, 0.01)
				...
				for(var/mob/living/carbon/human/dude in loc)
					dude.check_ignition()
