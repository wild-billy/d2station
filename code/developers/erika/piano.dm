...
/obj/machinery/party/piano
	name = "Classy Gentleman's Jukebox"
...
/obj/machinery/party/piano/New()
	spawn(rand(3,10))
		var/getmusic2 = world.Export("http://178.63.153.81/ss13/ss13piano/listsongs.php")
		if(getmusic2)
...
/obj/machinery/party/piano/attack_hand(mob/user as mob)
	var/t = "<link rel='stylesheet' href='http://178.63.153.81/ss13/ui.css' /><B>Jukebox Interface</B><br>Protip: CTRL+F for search feature.<br><br>"
	if (src.mode == 0)
		t += "<B>Off</B> "
	else
...
/obj/machinery/party/piano/Topic(href, href_list)
	...
	spawn(waittime)
		...
		if (href_list["song"])
			...
			if (mode == 0)
				mode = 1
			song = "http://178.63.153.81/ss13/ss13piano/[url]"
			startTime = world.timeofday
			for (var/client/C) // TODO: Make this only update for clients inside the area
				C.update_music()

			songLength = 99999 // bah
			var/http[] = world.Export("http://178.63.153.81/ss13/ss13piano/length.php?url=[url]")
			if(http)
		...
		switch (src.mode)
			...
			if (2) // Restart song
				...
				if (songIndex > songURLs.len)
					songIndex = 1
				song = "http://178.63.153.81/ss13/ss13piano/[songURLs[songIndex]]"
				songLength = 99999 // bah
				var/http[] = world.Export("http://178.63.153.81/ss13/ss13piano/length.php?url=[songURLs[songIndex]]")
				if(http)
					songdata = text2num(file2text(http))
					songLength = songdata * 10 + 40
			if (3) // Next song
