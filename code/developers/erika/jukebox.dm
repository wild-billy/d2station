...
/obj/machinery/party/turntable2/New()
	spawn(rand(3,10))
		var/getmusic = world.Export("http://178.63.153.81/ss13/ss13media/listsongs.php")
		if(getmusic)
...
/obj/machinery/party/turntable2/Topic(href, href_list)
	...
	spawn(waittime)
		...
		if (href_list["song"])
			...
			if (mode == 0)
				mode = 1
			song = "http://178.63.153.81/ss13/ss13media/[url]"
			startTime = world.timeofday
			for (var/client/C) // TODO: Make this only update for clients inside the area
				C.update_music()

			songLength = 99999 // bah
			var/http[] = world.Export("http://178.63.153.81/ss13/ss13media/length.php?url=[url]")
			if(http)
		...
		switch (src.mode)
			...
			if (2) // Restart song
				...
				if (songIndex > songURLs.len)
					songIndex = 1
				song = "http://178.63.153.81/ss13/ss13media/[songURLs[songIndex]]"
				songLength = 99999 // bah
				var/http[] = world.Export("http://178.63.153.81/ss13/ss13media/length.php?url=[songURLs[songIndex]]")
				if(http)
					songdata = text2num(file2text(http))
					songLength = songdata * 10 + 40
			if (3) // Next song
