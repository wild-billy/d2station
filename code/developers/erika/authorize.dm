/client/proc/authorize()
	...
	spawn(rand(1,5))
		...
		spawn (rand(4, 18))
			var/result = world.Export("http://api.d2k5.com/ss13/auth.php?ckey=[src.ckey]&isGold")
			var/success = 0
			if(result)
...
/client/proc/goonauth()
	...
	/*if (src.authenticating)
		return*/
	spawn(rand(1,5))
		var/feresult = world.Export("http://api.d2k5.com/ss13/auth.php?ckey=[src.ckey]&isCorrect")
		if(feresult)
