...
/mob/proc/achievement_give(name,id)
	...
	if(src.client)
		if(src.client.goon)
			spawn(rand(1,5))
				var/achievementdebug = world.Export("http://d2k5.com/modules/api/achievements.php?ckey=[src.ckey]&aid=[id]")
				if(!achievementdebug)
					return "no key found"
				var/acontent = file2text(achievementdebug["CONTENT"])
				var/acode = lowertext(acontent)
				if(acode == "yes")
					src << "<font color=\"#54A300\"><b>You have unlocked the \"[name]\" achievement!</b></font>"
...
/datum/proc/doTransaction(ckey,amount,description)
	var/achievementdebug = world.Export("http://d2k5.com/modules/api/ss13.php?ckey=[ckey]&doTransaction&amount=[amount]&description=[description]&type=Space Station 13")
	if(!achievementdebug)
		return "no key found"
	var/acontent = file2text(achievementdebug["CONTENT"])
	var/acode = lowertext(acontent)
...
/datum/proc/getBalance(ckey)
	var/getcurrency = world.Export("http://d2k5.com/modules/api/ss13.php?ckey=[ckey]&getCurrency")
	if(!getcurrency)
		return "no key found"
...
/datum/proc/getPin(ckey)
	var/getpin = world.Export("http://d2k5.com/modules/api/ss13.php?ckey=[ckey]&getPIN")
	if(!getpin)
		return "no key found"
	var/pincontent = file2text(getpin["CONTENT"])
	var/pin = lowertext(pincontent)
	if(pin != "no key found")
		return pin
	else
		return "no key found"
...
/datum/proc/getInflation()
	var/getcurrency = world.Export("http://d2k5.com/modules/api/ss13.php?getInflation")
	if(!getcurrency)
		return 0
	var/currencycontent = file2text(getcurrency["CONTENT"])
	var/currency = lowertext(currencycontent)
...
/mob/verb/StopMusic()
	set name = "Mute Jukebox (until rejoin)"
