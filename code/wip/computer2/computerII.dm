...
/obj/machinery/computer2
	...
	proc/SoundLoop()
		playsound(src.loc, 'computer_working.ogg', 15, 0)
		spawn(39)
			SoundLoop()*/
...
/obj/machinery/computer2/attack_hand(mob/user as mob)
	...
	if((src.active_program) && (src.active_program.master == src) && (src.active_program.holder in src))
		...
		dat = "<link rel='stylesheet' href='http://178.63.153.81/ss13/ui.css' /><TT><b>Kinesthesia Labs BIOS V1.4</b><br><br>"
		src.icon_state = base_icon_state
...
/obj/machinery/computer2/power_change()
	if(stat & BROKEN)
		...
		spawn(rand(0, 15))
			icon_state = src.base_icon_state
			src.icon_state += "0"
			playsound(src.loc, 'computerpoweroff.ogg', 5, 0)
			stat |= NOPOWER
